-- 1. Add profiles_viewed column to recruiter_profiles table if it doesn't exist
ALTER TABLE public.recruiter_profiles
  ADD COLUMN IF NOT EXISTS profiles_viewed integer DEFAULT 0;

-- 2. Create increment_recruiter_profiles_viewed security-definer function
CREATE OR REPLACE FUNCTION public.increment_recruiter_profiles_viewed(p_recruiter_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET profiles_viewed = COALESCE(profiles_viewed, 0) + 1
  WHERE id = p_recruiter_id;
END;
$$;

GRANT EXECUTE ON FUNCTION public.increment_recruiter_profiles_viewed(uuid) TO authenticated;

-- 3. Re-create get_org_members_with_stats function to return profiles_viewed
