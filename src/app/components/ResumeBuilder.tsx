...
function buildResumeHTML(
  ...
  const { basicInfo, summary, skills, experiences, education, projects, certifications, languages } = props;
  ...
  // SVGs with custom colored stroke
  ...
  if (templateId === "template-1") {
    ...
    const languagesHTML = languages.length > 0
      ? `<div style="margin-bottom:10px;font-size:11px;color:#444;">
          <strong>Languages:</strong> ${languages.map(l => `${l.language}${l.proficiency ? ` (${l.proficiency})` : ""}`).join(", ")}
        </div>`
      : "";
    ...
    return `
      <div id="resume-render-target" style="width:794px;height:1122px;font-family:'Segoe UI',Arial,Helvetica,sans-serif;background:#fff;color:#333;padding:45px;box-sizing:border-box;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between;">
        <div style="box-sizing:border-box;">
          ...
          <!-- Additional info -->
          ${(languagesHTML || certificationsHTML) ? `
            <div style="margin-top:14px;padding-top:10px;">
  ...
  if (templateId === "template-2") {
    ...
    const experienceHTML = experiences.map((exp, idx) => {
      const dateRange = `${exp.startMonth} ${exp.startYear} – ${exp.current ? "Present" : `${exp.endMonth} ${exp.endYear}`}`;
      const descLines = exp.description
        ? exp.description.split(/\n/).map(l => l.trim()).filter(l => l.length > 0)
    ...
    return `
      <div id="resume-render-target" style="width:794px;height:1122px;font-family:'Segoe UI',Arial,Helvetica,sans-serif;background:#fff;color:#333;box-sizing:border-box;overflow:hidden;display:flex;">
        <!-- Left Sidebar (Navy) -->
        ...
        <!-- Right Main Panel -->
        <div style="flex:1;box-sizing:border-box;display:flex;flex-direction:column;overflow:hidden;">
          <!-- Top Name Panel -->
          ...
          <!-- Timeline Sections -->
          <div style="padding:22px 30px;box-sizing:border-box;flex:1;overflow:hidden;">
            <!-- Profile -->
  ...
  if (templateId === "template-4") {
    ...
    return `
      <div id="resume-render-target" style="width:794px;height:1122px;font-family:'Segoe UI',Arial,Helvetica,sans-serif;background:#fff;color:#333;box-sizing:border-box;overflow:hidden;display:flex;">
        ...
        <!-- Right Main Panel -->
        <div style="flex:1;padding:30px;box-sizing:border-box;overflow:hidden;">
  ...
  if (templateId === "template-5") {
    ...
    return `
      <div id="resume-render-target" style="width:794px;height:1122px;font-family:'Segoe UI',Arial,Helvetica,sans-serif;background:#fff;color:#333;box-sizing:border-box;overflow:hidden;display:flex;flex-direction:column;padding:30px;justify-content:space-between;">
        <div style="box-sizing:border-box;">
          ...
          <!-- Two Column Body -->
          <div style="display:flex;gap:30px;box-sizing:border-box;flex:1;overflow:hidden;">
            <!-- Left Sidebar -->
  ...
  const languagesHTML = languages.length > 0
    ? `<div style="margin-top:14px;padding-top:10px;border-top:1.5px solid #1a4d8f;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:12px;">
          ${languages.map(l => `<span style="font-size:11px;color:#444;">• ${l.language}${l.proficiency ? ` (${l.proficiency})` : ""}</span>`).join("")}
        </div>
      </div>`
    : "";
  ...
  return `
    <div id="resume-render-target" style="width:794px;height:1122px;font-family:'Segoe UI',Arial,Helvetica,sans-serif;background:#fff;color:#333;position:relative;box-sizing:border-box;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between;">
      <div style="box-sizing:border-box;">
        <!-- Header -->
        <div style="background:linear-gradient(135deg,#1a4d8f 0%,#0d3567 100%);padding:28px 36px 20px;display:flex;align-items:center;justify-content:space-between;position:relative;box-sizing:border-box;">
          <!-- Decorative pattern -->
          ...
          <div style="position:relative;z-index:1;flex:1;padding-right:20px;box-sizing:border-box;overflow:hidden;">
            ...
            <div style="margin-top:14px;display:flex;flex-wrap:wrap;gap:8px 20px;">
              <span style="display:inline-flex;align-items:center;gap:5px;color:rgba(255,255,255,0.95);font-size:11px;">${getPhoneIcon()} ${basicInfo.phone}</span>
              ${websiteDisplay ? `<span style="display:inline-flex;align-items:center;gap:5px;color:rgba(255,255,255,0.95);font-size:11px;">${getGlobeIcon()} ${websiteDisplay}</span>` : ""}
              <span style="display:inline-flex;align-items:center;gap:5px;color:rgba(255,255,255,0.95);font-size:11px;">${getLocationIcon()} ${basicInfo.location}</span>
              <span style="display:inline-flex;align-items:center;gap:5px;color:rgba(255,255,255,0.95);font-size:11px;">${getEmailIcon()} ${basicInfo.email}</span>
            </div>
          </div>
...
export default function ResumeBuilder(props: ResumeBuilderProps) {
  ...
  const handlePreviewClick = () => {
    ...
    if (!newWindow) {
      alert("Popup blocker is enabled. Please allow popups to view the resume preview.");
      return;
    }

    const htmlContent = buildResumeHTML(props, resolvedProfilePic, selectedTemplate);
    ...
    newWindow.document.write(`
      ...
      <html lang="en">
        <head>
          <meta charset="UTF-8">
          <title>Resume Preview - ${props.basicInfo.name}</title>
          <style>
            body {
              margin: 0;
              padding: 40px 20px;
